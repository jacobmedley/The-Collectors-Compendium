...
const SEED_ITEMS = [
  ...
  { id: 'rogue-gambit', name: 'Rogue & Gambit Premium Format', franchise: 'Marvel (X-Men)', studio: 'Sideshow Collectibles', category: 'Premium Format Figure', edition: 'Limited Edition', rarityTier: 'T4', productionRun: 1200, msrp: 750, itemCost: 750, totalPaid: 840.84, marketLow: 700, marketMid: 900, marketHigh: 1100, marketSource: 'estimated', liquidity: 'high', savingsMethods: [], notes: 'Standalone purchase. $750 item + $38 ship. X-Men pair = strong demand.', url: '', imageUrl: 'https://raw.githubusercontent.com/jacobmedley/The-Collectors-Compendium/main/images/thumbs/rogue-gambit.jpg', purchaseDate: '2025-01-12', releaseDate: null, editionNumber: null, acquisitionDate: '2025-03-27', dimensions: { heightIn: 11.5, widthIn: 21.75, depthIn: 28.25, weightLbs: null }, status: 'owned' },
...
export default function CollectionApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get('collection-overrides');
  ...
  const saveOverrides = async (next) => {
    setOverrides(next);
    try { await window.storage.set('collection-overrides', JSON.stringify(next)); } catch (e) {}
  };
